...
const TaxCalculator = () => {
  ...
  const chartData = useMemo(() => {
    return Array.from({ length: holdingYears + 1 }, (_, i) => {
      const year = i;
      const { oldTax, newTax, indexedCost } = calculateTaxes(
        year,
        priceMultiplier,
        indexationRate
      );
  ...
  return (
    <div style={{ maxWidth: "800px", margin: "0 auto", padding: "20px" }}>
      ...
      <ResponsiveContainer width="100%" height={400}>
        ...
        >
          ...
          <Line
            type="monotone"
            dataKey="oldTax"
            name="Old Tax (20% with indexation)"
            stroke="#8884d8"
            strokeWidth={3}
            dot={false}
          />
      ...
      <div style={{ marginTop: "20px", fontSize: "14px" }}>
        <p>
          Base property value: ₹{BASE_PRICE.toLocaleString("en-IN")} (1 Crore)
        </p>
        <p>
          Graph shows tax amounts and indexed cost over the selected holding
          period.
        </p>
        <p>
          Adjust sliders to see how final price multiplier, indexation rate, and
          holding period affect taxes.
        </p>
        {crossoverPoint ? (
          <div style={{ fontWeight: "bold", marginTop: "10px" }}>
            ...
            <p>
              - Sell Price: ₹
              {(BASE_PRICE * priceMultiplier).toLocaleString("en-IN")}
            </p>
            <p>
              - Indexed Cost: ₹
              {crossoverPoint.indexedCost.toLocaleString("en-IN")}
            </p>
